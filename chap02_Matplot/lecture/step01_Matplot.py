...
# 여러가지 색상 산점도
cdata = np.random.randint(1, 4, size=10)
...
# data 생성
data3 = np.random.randint(1, 100, 100)  # 1 ~ 99
data4 = np.random.randint(11, 110, 100)  # 1 ~ 99
...
chart = fig.add_subplot()  # 기본 인수 : (1,1,1)
...
plt.xlabel('데이터')
plt.ylabel('난수 정수')
plt.legend(loc='best')  # 적절한 위치에 범례 추가
plt.show()
...
plt.title('2020년 1분기 전국 지역별 판매 현황')
plt.show()